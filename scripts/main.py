...
def arg_parser():
    ...
    parser.add_argument(
        "--dataset", type=str, choices=["Uyghur", "temp", "Aishell"],
        help="Dataset for running DialectCLIP"
    )
    parser.add_argument(
        "--pretrain", action="store_true",
        help="Whether to pretrain on Aishell corpus or not"
    )
    ...
    model_parser.add_argument(
        "--lora_dropout_rate", default=0.01, type=float, help="dropout rate in Lora adapter"
    )
    model_parser.add_argument(
        "--lora_alpha", default=8, type=int, help="temperature hyperparamter in Lora"
    )
    model_parser.add_argument(
        "--attn_dropout_rate", type=float, default=0.01,
        help="Dropout rate in the attention layer when computing the attention matrix"
    )
    model_parser.add_argument(
        "--mlp_dropout_rate", type=float, default=0.01,
...
def main():
    ...
    model_config.lora_dropout_rate = args.lora_dropout_rate if args.lora_dropout_rate is not None else model_config.lora_dropout_rate
    ...
    if args.dataset == "Uyghur":
        ...
        dataset = Aishell_Dataset(file_path="./Data/Aishell")
    else:
